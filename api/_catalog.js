...
function priceLine(item) {
  ...
  if (sub) {
    ...
    return { productId: sub.productId, unitPrice: round2(product.price * (1 - discount)) };
  }
  const product = PRODUCTS[item.id];
  if (!product) throw new Error(`Unknown product ${item.id}`);
  return { productId: item.id, unitPrice: product.price };
}
...
export function priceCart(cart) {
  ...
  for (const item of cart) {
    ...
    const { productId, unitPrice } = priceLine(item);
    subtotal += unitPrice * qty;
    priced.push({ productId, unitPrice, qty });
  }
